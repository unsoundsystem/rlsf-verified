...
impl<'pool, const FLLEN: usize, const SLLEN: usize> Tlsf<'pool, FLLEN, SLLEN> {
    ...
    pub const fn new() -> Self {
        Self {
            ...
            gs: GhostTlsf {
                ghost_free_list: Seq::new(FLLEN, |i: int| Seq::new(SLLEN, |j: int| None)),
    ...
    #[inline]
    fn map_floor(size: usize) -> (usize, usize)
    requires Self::valid_block_size(size),
    ensures
        Self::valid_block_index(r),
    ...
    const fn max_pool_size() -> (r: usize)
        requires
            usize::BITS > Self::granularity_log2() + FLLEN as u32
        ensures
        ...
        // Tlsf is well-formed
    {
        ...
        while size_remains >= GRANULARITY * 2 /* header size + minimal allocation unit */ {
            ...
            let tracked next_free_perm = self.gs@.ghost_free_list[fl][sl];
