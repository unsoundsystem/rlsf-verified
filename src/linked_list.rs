...
//        }
                ...
                ==> final(self).shadow_freelist@.m[bi] == old(self).shadow_freelist@.m[bi]
        {
            ...
            if self.first_free[idx.0][idx.1] != null_bhdr() {
                ...
                {
                    #[cfg(feature = "rdpmc-bench")]
                    perf_probe::begin(perf_probe::ProbeSlot::FirstFreeRead);
                    let n = ptr_ref(first_free_link, Tracked(&first_free_fl_pt)).next_free;
                    #[cfg(feature = "rdpmc-bench")]
                    perf_probe::end(perf_probe::ProbeSlot::FirstFreeRead);
                    #[cfg(feature = "rdpmc-bench")]
                    perf_probe::begin(perf_probe::ProbeSlot::FirstFreeWrite);
                    ptr_mut_write(first_free_link, Tracked(&mut first_free_fl_pt), FreeLink {
                        next_free: n,
                        prev_free: node
                    });
                    #[cfg(feature = "rdpmc-bench")]
                    perf_probe::end(perf_probe::ProbeSlot::FirstFreeWrite);
                }
                ...
                #[cfg(feature = "rdpmc-bench")]
                perf_probe::end(perf_probe::ProbeSlot::NewNodeWrite);
                ...
                self.set_freelist(idx, node);
                #[cfg(feature = "rdpmc-bench")]
                perf_probe::begin(perf_probe::ProbeSlot::EmptyBranchWrite);
                ptr_mut_write(get_freelink_ptr(node), Tracked(&mut node_fl_pt), FreeLink {
                    next_free: null_bhdr(),
                    prev_free: null_bhdr()
                });
                #[cfg(feature = "rdpmc-bench")]
                perf_probe::end(perf_probe::ProbeSlot::EmptyBranchWrite);
                // {{{ proof block
                proof {
                    self.all_blocks.perms.borrow_mut().tracked_insert(node, BlockPerm {
                        points_to: node_blk.points_to,
            ...
            #[cfg(feature = "rdpmc-bench")]
            perf_probe::end(perf_probe::ProbeSlot::Whole);
        }
